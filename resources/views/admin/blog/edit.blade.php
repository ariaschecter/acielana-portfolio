    ...
    <div class="card">
        <div class="card-body flex flex-col p-6">
            ...
            <div class="card-text h-full ">
                <form class="space-y-4" method="POST" action="{{ route('admin.blog.update', $blog->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" class="select2 form-control w-full mt-2 py-2">
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ $category->id == $blog->category_id ? 'selected' : '' }} class=" inline-block font-Inter font-normal text-sm text-slate-600">{{ $category->category_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-area">
                        <label for="blog_picture" class="form-label">Picture</label>
